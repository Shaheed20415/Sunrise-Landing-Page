...
if ($_POST) {
    ...
    switch ($action) {
        ...
        case 'mark_progress':
            $query = "UPDATE contact_submissions SET status = 'in_progress' WHERE id = :id";
            $stmt = $db->prepare($query);
            $stmt->bindParam(':id', $id);
            $stmt->execute();
            break;
...
<head>
    ...
    <title>Contact Data - Admin Dashboard</title>
...
<body class="bg-gray-100">
    ...
    <div class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-orange-600 to-amber-600 shadow-xl">
        ...
        <nav class="mt-8">
            ...
            <a href="contact-data.php" class="flex items-center px-6 py-3 text-white bg-black bg-opacity-20 border-r-4 border-white">
        ...
        <header class="bg-white shadow-lg">
            <div class="flex items-center justify-between px-6 py-4">
                <h2 class="text-2xl font-bold text-gray-800">
                    <i class="fas fa-phone mr-2 text-green-600"></i>Get in Touch Form Submissions
                </h2>
                <div class="flex items-center space-x-4">
                    <span class="text-gray-600">Total: <?php echo count($contacts); ?> submissions</span>
                </div>
            </div>
        </header>
                            ...
                            <?php else: ?>
                                <?php foreach ($contacts as $contact): ?>
                                    <tr class="hover:bg-gray-50 transition-colors duration-300">
                                        <td class="px-6 py-4">
                                            <div class="font-semibold text-gray-800"><?php echo htmlspecialchars($contact['name']); ?></div>
                                        </td>
                                        <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($contact['email']); ?></td>
                                        <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($contact['phone']); ?></td>
                                        <td class="px-6 py-4 text-gray-600">
                                            <div class="max-w-xs truncate"><?php echo htmlspecialchars(substr($contact['message'], 0, 50)) . '...'; ?></div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?php
                                            $status_colors = [
                                            ...
                                            ?>
                                            <span class="px-3 py-1 rounded-full text-xs font-semibold <?php echo $status_colors[$contact['status']]; ?>">
                                                <?php echo ucfirst(str_replace('_', ' ', $contact['status'])); ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-gray-600 text-sm"><?php echo date('M j, Y', strtotime($contact['created_at'])); ?></td>
                                        <td class="px-6 py-4">
                                            <div class="flex space-x-2">
                                                <button onclick="viewDetails(<?php echo $contact['id']; ?>, 'contact')" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-lg text-xs transition-colors duration-300">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                                <button onclick="editRecord(<?php echo $contact['id']; ?>, 'contact')" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-lg text-xs transition-colors duration-300">
